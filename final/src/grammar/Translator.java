...
public class Translator {
	...
	public Translator (FileInputStream input) {
	...
	private Node primaryExpression() throws SyntaxError, IOException {
		Node node;
		Object result;
		
		if (token.getTag() == Tag.ID) {
			check(Tag.ID);
			result = current.lookup(((Word) token).getLexeme());
			if (result instanceof Double) {
				return new NumericIdentifier(((Word) token).getLexeme());
			} else if (result instanceof Boolean) {
				return new BooleanIdentifier(((Word) token).getLexeme());
			} 
		} else if (token.getTag() == Tag.NUMBER) {
			check(Tag.NUMBER);
			return new NumberValue(((Number) token).getValue());
